...
def draw_screen(screen, snake, fruit, grid):
    ...
    screen.blit(score_text, (WIDTH // 4 - score_text.get_width() // 2,
     HEIGHT - GAME_BAR_HEIGHT // 2 - score_text.get_height() // 2))
    record_text = SCORE_FONT.render("Record: " + str(snake.record), 1, WHITE)
    screen.blit(record_text, (WIDTH - WIDTH // 4 - record_text.get_width() // 2, 
...
def draw_lost_text(screen):
    pygame.draw.rect(screen , WHITE, (PIXEL_SIZE * 6, PIXEL_SIZE * 6, HEIGHT - PIXEL_SIZE * 12, WIDTH - PIXEL_SIZE * 12))
    lost_text = LOST_FONT.render("Your score: " + str(snake.fruits_eaten), 1, BLACK)
    screen.blit(lost_text, (WIDTH // 2 - lost_text.get_width() // 2, 
    HEIGHT // 2 - lost_text.get_height() // 2))

def lost_game(snake):
    if snake.body[0].x < 0 or snake.body[0].x + SNAKE_VELOCITY > WIDTH:
        return True
    if snake.body[0].y < 0 or snake.body[0].y + SNAKE_VELOCITY > HEIGHT - GAME_BAR_HEIGHT:
        return True
